...
public class ToggleSwitchGroupManager : MonoBehaviour
{
    ...
    private IEnumerator DelayedStart()
    {
        ...
        Debug.Log("ToggleSwitchGroupManager running Start()");

        bool areAllToggledOff = true;
        foreach (var button in _toggleSwitches)
        {
            Debug.Log($"{button.gameObject.name} initial state: {button.CurrentValue}");
        ...
        {
            Debug.Log("Forcing a toggle ON due to group manager settings.");

            if (initialToggleSwitch != null)
                initialToggleSwitch.ToggleByGroupManager(true);
            else if (_toggleSwitches.Count > 0)
